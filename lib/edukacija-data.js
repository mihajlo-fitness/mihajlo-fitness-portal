...
export const KATEGORIJE = [
  ...
  },
  {
    ...
    lekcije: [
      { slug: "da-li-ti-je-kardio-potreban", naslov: "Da li ti je kardio zaista potreban", trajanje: "1:27", opis: "Iskren pregled kad kardio pomaže, a kad je opciono — u zavisnosti od tvog cilja.", kljucneTacke: ["Kardio nije obavezan za mršavljenje ako je kalorijski deficit ispoštovan", "Za srce, izdržljivost i opšte zdravlje, ipak ima jasnu vrednost", "Ne mora da bude trčanje — bilo koja aktivnost koja podiže puls računa se"], video: "https://youtube.com/shorts/bzTYQD3rw90" },
  ...
  },
  {
    ...
    lekcije: [
      { slug: "zasto-je-san-bitniji-od-treninga", naslov: "Zašto je san bitniji od dodatnog treninga", trajanje: "1:35", opis: "Veza između sna i napretka koju većina početnika ignoriše dok ne udari u zid.", kljucneTacke: ["Mišić raste dok se odmara, ne dok se vežba", "Loš san direktno utiče na apetit i odluke o ishrani sledećeg dana", "7-9 sati sna je realan cilj za većinu odraslih"], video: "https://youtube.com/shorts/_Cwmhr60yQY" },
...
// Sabira trajanja SAMO lekcija koje imaju pravi video (ne placeholder "#").
// Vraća null ako nijedna lekcija u kategoriji još nema video.
export function ukupnoTrajanjeKategorije(kategorija) {
  const saVideom = kategorija.lekcije.filter((l) => l.video && l.video !== "#");
  if (saVideom.length === 0) return null;

  let ukupnoSekundi = 0;
  for (const lek of saVideom) {
