...
export default function JavniKontaktPage() {
  ...
  const [emailPosiljalac, setEmailPosiljalac] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    notifyCoach(
      ...
      `${ime} (${emailPosiljalac}) je poslao/la pitanje preko Kontakt stranice.\n\nPoruka: ${poruka || "—"}`
    );
    setSubmitting(false);
    setSent(true);
  };
          ...
          {sent ? (
            ...
            <div>
              ...
              <div className="[&_input]:bg-white/[0.03] [&_input]:border-white/10 [&_input]:text-white [&_textarea]:bg-white/[0.03] [&_textarea]:border-white/10 [&_textarea]:text-white [&_label]:text-white/50">
                ...
                <Field label="Email ili broj telefona">
                  <TextInput
                    value={emailPosiljalac}
                    onChange={(e) => setEmailPosiljalac(e.target.value)}
                    placeholder="tvoj@email.com ili 06x xxx xxxx"
                  />
                </Field>
                <Field label="Poruka">
              ...
              <button
                ...
                disabled={!ime.trim() || !emailPosiljalac.trim() || !poruka.trim() || submitting}
