...
export default function LekcijaPage({ params }) {
  ...
  if (!found) {
    return (
      <div className="max-w-md mx-auto px-6 py-16 text-center">
        ...
        <Link href="/app/edukacija" className="text-accent font-semibold text-[14px]">
          ← Nazad na Edukaciju
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-md md:max-w-2xl mx-auto px-6 py-10 animate-fade-in">
      <button
        onClick={() => router.push(`/app/edukacija/${kategorija.slug}`)}
      ...
      <div className="rounded-[28px] overflow-hidden mb-6 bg-black relative aspect-[9/16] max-w-[420px] mx-auto">
        {video?.type === "youtube" || video?.type === "vimeo" ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-accent/25 to-accent/5 flex flex-col items-center justify-center gap-2">
            <PlayCircle size={44} className="text-white/70" />
            <span className="text-white/70 text-[13px] font-medium">Video uskoro — tekst lekcije je ispod</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-12">
        {prethodna ? (
          <Link
            href={`/app/edukacija/${kategorija.slug}/${prethodna.slug}`}
        ...
        {sledeca ? (
          <Link
            href={`/app/edukacija/${kategorija.slug}/${sledeca.slug}`}
        ...
        <div>
          ...
          <div className="space-y-2.5">
            {slicne.map((lek) => (
              <Link
                key={lek.slug}
                href={`/app/edukacija/${kategorija.slug}/${lek.slug}`}
